...
def native():
    ...
    if not dsn or not os.environ.get("OSB_ITEM_OBSERVATION_FIXTURE_CONTAINER", "").startswith("codex-osb-item-community-"):
    ...
    assert db.cypher_query("MATCH (n) RETURN count(n)")[0] == [[0]], "Fixture graph must be empty"
    f = fixture()
    nonce = str(uuid4())
    p = {**f.request.model_dump(), "tenantId": TENANT, "fixture": nonce,
         "decision": canonical_json(f.blobs[0]), "candidate": canonical_json(f.blobs[1]),
         "source": canonical_json(f.blobs[2]), "evidence": canonical_json(f.blobs[3]),
         "context": canonical_json(f.blobs[4]), "artifact": canonical_json(f.blobs[5]),
         "operation": canonical_json(f.blobs[6]), "operationHash": f.applied["payload"]["evidenceRecords"][0]["payloadHash"]["value"]}
    db.cypher_query("""
      ...
      CREATE (study:StudyRoot {fixture:$fixture,uid:$nativeStudyId})
        -[:LATEST {version:$nativeStudyVersion,status:'Draft'}]->(:StudyValue {fixture:$fixture})
    ...
    """, p)
    f.service = NativeItemObservationService(auth_reader=lambda: f.auth, clock=lambda: f.now[0], monotonic=lambda: f.now[0])
    f.params = p
    try:
        yield f
    finally:
        # Delete only this authored fixture's nodes after verifying membership.
        assert db.cypher_query("MATCH (n) WHERE n.fixture IS NULL OR n.fixture<>$fixture RETURN count(n)", p)[0] == [[0]]
...
def test_actual_native_queries_return_current_typed_bytes(native):
    ...
    assert result["operationHash"] == native.applied["payload"]["evidenceRecords"][0]["payloadHash"]


@pytest.mark.parametrize("mutation", [
    "MATCH (b:PlatformNativeStudyBinding) SET b.status='retired'",
    "MATCH (s:DomainStudyScope) SET s.tenant_id='foreign-tenant'",
    "MATCH (:StudyRoot)-[h:LATEST]->() SET h.version='0.2'",
...
def test_actual_current_version_authority_context_and_bounds_refuse(native, mutation):
    ...
    mutation = mutation.replace("repeat('x',65537)", "$largeJson").replace("repeat('x',4097)", "$largeField")
    db.cypher_query(mutation, {**native.params, "largeJson": "x" * 65537, "largeField": "x" * 4097})
...
def test_actual_context_withdrawal_between_observations(native):
    class Withdrawal(NativeItemObservationRepository):
        ...
        def item(self, p, timeout):
            ...
            self.count += 1
            if self.count == 1:
                db.cypher_query("MATCH (c:OsbMappingContextSnapshot {context_hash:$mappingContextHash}) DELETE c", p)
            return result
...
def test_actual_scope_withdrawal_during_last_item_read(native):
    class Withdrawal(NativeItemObservationRepository):
        ...
        def item(self, p, timeout):
            ...
            self.count += 1
            if self.count == 2:
                db.cypher_query("MATCH (s:DomainStudyScope {study_uid:$nativeStudyId}) SET s.status='quarantined'", p)
            return result
    native.service.repository = Withdrawal()
