...
from clinical_mdr_api.tests.integration.services.test_native_item_observation_neo4j import native
from clinical_mdr_api.tests.unit.services.test_selected_activity_item_observation import selection_request
...
def selected(native):
    ...
    db.cypher_query("""
      ...
      CREATE (study)-[:HAS_VERSION {version:$studyValueVersion,status:'Draft'}]->(studyValue)
    ...
    """, f.params)
    f.service = SelectedActivityItemService(auth_reader=lambda: f.auth, clock=lambda: f.now[0], monotonic=lambda: f.now[0])
    return f
...
def test_actual_one_current_selected_full_path(selected):
    ...
    assert value["semanticApprovalVerified"] is False and value["requirednessVerified"] is False
...
def test_actual_missing_changed_duplicate_unversioned_or_unsupported_path_refuses(selected, mutation):
    db.cypher_query(mutation, {**selected.params, "oversized": "x" * 4097})
    with pytest.raises(NativeItemObservationError): selected.service.observe(selected.request)
...
def test_duplicate_introduced_after_independent_count_is_still_refused(selected):
    class Duplicate(SelectedActivityItemRepository):
        ...
        def path_count(self, p, timeout):
            ...
            self.calls += 1
            if self.calls == 1:
                db.cypher_query("MATCH (s:StudyActivityInstance)-[:HAS_SELECTED_ACTIVITY_INSTANCE]->(a) CREATE (s)-[:HAS_SELECTED_ACTIVITY_INSTANCE]->(a)")
            return rows
