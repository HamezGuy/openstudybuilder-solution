...
def test_native_midnight_package_storage_resolves_without_losing_original_evidence():
    source, selection, epoch, rows = fixture()
    for package in (rows[0][0], rows[0][2]):
        package["effective_date"] += "T00:00:00.000000000+00:00"
    witness = resolve(source, selection, epoch, rows)
    assert epoch.epoch.sponsor_preferred_name == "float"
    assert not witness["issues"]
    assert witness["package"]["selectedPackage"]["effective_date"].endswith(
        "T00:00:00.000000000+00:00"
    )


@pytest.mark.parametrize("suffix", [
    "T12:00:00+00:00", "T00:00:00.000000001+00:00", "T00:00:00+05:30",
])
def test_package_storage_with_a_real_time_component_is_not_coerced_to_a_date(suffix):
    source, selection, epoch, rows = fixture()
    rows[0][0]["effective_date"] += suffix
    witness = resolve(source, selection, epoch, rows)
    assert epoch.epoch.sponsor_preferred_name is None
    assert "STUDY_EPOCH_CT_PACKAGE_DATE_INVALID" in witness["issues"]
