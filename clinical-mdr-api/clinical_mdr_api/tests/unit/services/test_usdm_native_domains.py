...
def mapping(graph, monkeypatch):
    ...
    monkeypatch.setattr(
        "clinical_mdr_api.services.ddf.usdm_mapper.db.cypher_query", source.query
    )
...
def nodes(value):
    if isinstance(value, dict):
        if isinstance(value.get("instanceType"), str) and isinstance(
            value.get("id"), str
        ):
            yield value
...
def test_actual_native_models_map_all_new_domains_without_mutating_sources(monkeypatch):
    ...
    assert {
        ...
        "activityInstanceDefinition",
        "studySnapshotHistory",
        "studyProtocolHeader",
    ...
    retained = next(
        ...
        if row["kind"] == "activityInstanceDefinition"
    )
...
def test_sex_design_intent_and_blinding_use_codes_not_sponsor_labels(monkeypatch):
    ...
    assert [entry["code"] for entry in mapped["population"]["plannedSex"]] == [
        "C16576",
        "C20197",
    ]
...
def test_unknown_health_is_omitted_and_explicit_booleans_are_not_changed(
    monkeypatch, healthy
):
    graph = native_study_graph()
...
def test_branch_cells_and_cohorts_keep_exact_selection_relationships(monkeypatch):
    ...
    assert parent["dataOriginType"]["code"] == "C176263"
    assert parent["dataOriginType"]["decode"] == "Synthetic Data"
    assert parent["dataOriginType"]["codeSystemVersion"] == "DDF CT 2025-09-26"
    assert parent["dataOriginDescription"] == graph["arms"][0].data_origin_description
    # A branch has no separately authored origin; do not copy the parent's
    # authority into this distinct selection.
    assert "dataOriginType" not in branch


@pytest.mark.parametrize("origin_uid", [None, "not-in-the-pinned-native-library"])
def test_arm_origin_requires_explicit_selected_authority_in_shared_fixture(
    monkeypatch, origin_uid
):
    graph = native_study_graph()
    graph["arms"][0].data_origin_type_uid = origin_uid
    expected = (
        "USDM_ARM_DATA_ORIGIN_AUTHORITY_REQUIRED"
        if origin_uid is None
        else "USDM_ARM_DATA_ORIGIN_CT_PIN_REQUIRED"
    )
    with pytest.raises(USDMMappingAuthorityRequired, match=expected):
        run_mapping(monkeypatch, graph)


def test_population_count_has_no_unit_and_cohort_counts_remain_exact_source_values(
    monkeypatch,
):
    report, *_ = run_mapping(monkeypatch)
    population = design(report)["population"]
    ...
    assert all(
        cohort["plannedEnrollmentNumber"] is None for cohort in population["cohorts"]
    )
    assert [
        row["record"]["number_of_subjects"]
        for row in report["nativeRecords"]
        if row["kind"] == "studyCohort"
    ] == [0, 20]
    assert any(
        issue["code"] == "USDM_ENROLLMENT_SCOPE_REVIEW_REQUIRED"
        for issue in report["mappingReport"]["issues"]
    )
...
def test_complete_cohort_counts_preserve_zero_without_a_count_unit(monkeypatch):
    ...
    assert [
        cohort["plannedEnrollmentNumber"]["value"] for cohort in population["cohorts"]
    ] == [0, 20]
    assert all(
        cohort["plannedEnrollmentNumber"].get("unit") is None
        for cohort in population["cohorts"]
    )
    assert not any(
        issue["code"].startswith("USDM_ENROLLMENT_SCOPE")
...
def test_partial_cohort_counts_are_not_completed_with_an_invented_zero(monkeypatch):
    ...
    assert any(
        issue["code"] == "USDM_COHORT_ENROLLMENT_INCOMPLETE"
        for issue in report["mappingReport"]["issues"]
    )


@pytest.mark.parametrize(
    "field", ["study_epoch_uid", "study_element_uid", "study_branch_arm_uid"]
)
def test_design_cell_cannot_mint_a_reference_to_an_absent_selected_entity(
    monkeypatch, field
):
    graph = native_study_graph()
    ...
    raw = next(
        row["record"]
        for row in report["nativeRecords"]
        if row["kind"] == "studyDesignCell"
    )
    assert raw[field] == "Absent_selection"
    assert any(
        issue["code"]
        in {"USDM_CELL_BRANCH_UNRESOLVED", "USDM_CELL_SELECTION_UNRESOLVED"}
        for issue in report["mappingReport"]["issues"]
    )


def test_soa_group_is_a_typed_activity_with_exact_children_and_source_metadata(
    monkeypatch,
):
    report, *_ = run_mapping(monkeypatch)
    activities = design(report)["activities"]
    soa = next(
        activity for activity in activities if activity["name"] == "Study assessments"
    )
    group = next(
        activity for activity in activities if activity["name"] == "Assessments"
    )
    assert soa["childIds"] == [group["id"]]
    source = next(
        row["record"]
        for row in report["nativeRecords"]
        if row["kind"] == "studySoAGroup"
    )
    assert source["study_activity_group_uids"] == ["Group_1"]
    assert source["show_soa_group_in_protocol_flowchart"] is False


def test_instance_instruction_and_footnote_use_selected_versions_and_exact_targets(
    monkeypatch,
):
    ...
    leaf = next(
        activity
        for activity in mapped["activities"]
        if activity["biomedicalConceptIds"]
    )
    parent = next(
        activity
        for activity in mapped["activities"]
        if leaf["id"] in activity["childIds"]
    )
    ...
    assert (
        parent["notes"][0]["text"]
        == "Collect before dosing; preserve <b>fasting</b> text."
    )
    assert leaf["notes"][0]["text"] == "Use <i>local</i> sample handling."
    assert version["notes"][0]["text"] == "Use <i>local</i> sample handling."
    second = next(
        encounter
        for encounter in mapped["encounters"]
        if encounter["label"] == "Visit 2"
    )
    first = next(
        encounter
        for encounter in mapped["encounters"]
        if encounter["label"] == "Visit 1"
    )
...
def test_timeline_has_entry_exit_and_uses_actual_non_global_anchor(monkeypatch):
    ...
    assert [entry["defaultConditionId"] for entry in instances[:-1]] == [
        entry["id"] for entry in instances[1:]
    ]
    ...
    assert (
        timeline["timings"][2]["relativeFromScheduledInstanceId"] == instances[2]["id"]
    )
    assert timeline["timings"][2]["relativeToScheduledInstanceId"] == instances[1]["id"]
    leaf = next(
        activity
        for activity in design(report)["activities"]
        if activity["biomedicalConceptIds"]
    )
...
def test_metadata_lock_does_not_invent_document_approval_or_future_history(monkeypatch):
    ...
    retained_history = [
        row["record"]["metadata_version"]
        for row in report["nativeRecords"]
        if row["kind"] == "studySnapshotHistory"
    ]
    assert retained_history == ["1.0", VERSION]
    assert any(
        activity["name"] == "Diagnosis" for activity in design(report)["activities"]
    )
...
def test_explicit_version_never_falls_back_to_reader_without_version_support(
    monkeypatch,
):
    graph = native_study_graph()
    mapper, _ = mapping(graph, monkeypatch)
    ...
    with pytest.raises(
        USDMMappingAuthorityRequired, match="SELECTED_VERSION_READER_REQUIRED"
    ):
        mapper.map_with_report(graph["study"], VERSION)
...
def test_partial_native_pages_fail_and_leave_mapper_reusable(monkeypatch, kind):
    ...
    mapper, _ = mapping(graph, monkeypatch)
    attribute = (
        "_get_osb_study_objectives" if kind == "selection" else "_get_snapshot_history"
    )
    ...
    with pytest.raises(
        USDMMappingAuthorityRequired, match="SOURCE_COLLECTION_TRUNCATED"
    ):
        mapper.map_with_report(graph["study"], VERSION)
...
def test_pinned_model4_schema_accepts_only_explicit_test_author_resolution(monkeypatch):
    ...
    assert (
        "isApproximate"
        not in draft["versions"][0]["studyDesigns"][0]["population"]["plannedAge"]
    )
    ...
    protocol["versions"][0]["status"] = code(
        "DocumentStatus_1", "Explicit author status"
    )
    for concept_index, concept in enumerate(
        reviewed["versions"][0]["biomedicalConcepts"]
    ):
        concept["code"] = {
            "id": f"ConceptAlias_{concept_index}",
        ...
        for property_index, prop in enumerate(concept["properties"]):
            prop.update(
                name="Explicit author acquisition property",
                datatype="float",
                isRequired=True,
                isEnabled=True,
                code={
                    "id": f"PropertyAlias_{concept_index}_{property_index}",
                    "instanceType": "AliasCode",
                    "standardCode": code(
                        f"Property_{concept_index}_{property_index}",
                        "Explicit author property",
                    ),
                },
            )
    ...
    for review in cell_reviews:
        ...
        decision = next(
            instance
            for instance in pending["instances"]
            if instance["id"] == review["draftTargets"]["scheduledDecisionInstanceId"]
        )
        assignment = next(
            item
            for item in decision["conditionAssignments"]
    ...
    for issue in report["mappingReport"]["issues"]:
        ...
        for segment in path[:-1]:
            target = (
                target[int(segment)] if isinstance(target, list) else target[segment]
            )
