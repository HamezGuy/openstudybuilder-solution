...
from clinical_mdr_api.services.ddf.usdm_ct_package_mapping import _release_date, _source_json
    ...
    if len(packages) != 1:
        ...
        if len(rows) != 1 or len(rows[0]) != 5:
            ...
            if (
                ...
                try:
                    dates = [date.fromisoformat(_release_date(row["effective_date"])) for row in ancestry]
                    if dates[0].isoformat() != package["effective_date"]:
                        raise ValueError("The selected release date differs from its stored package")
