...
def _release_date(value):
    ...
    text = str(value)
    # Older package imports stored DateProperty values as UTC-midnight
    # DateTime nodes. Normalize only that lossless date representation;
    # arbitrary timestamps must not silently become release dates. The raw
    # properties remain unchanged in the package evidence.
    match = re.fullmatch(r"(\d{4}-\d{2}-\d{2})T00:00:00(?:\.0+)?(?:Z|\+00:00)", text)
    return date.fromisoformat(match[1] if match else text).isoformat()
