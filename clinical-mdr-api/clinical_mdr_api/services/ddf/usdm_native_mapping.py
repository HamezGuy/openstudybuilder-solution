...
class NativeStudyMapping:
    ...
    def timelines(self, study):
        ...
        has_untimed_visits = any(
            getattr(getattr(visit, "timing_mode", None), "value", getattr(visit, "timing_mode", None)) == "UNTIMED"
            for visit in visits
        )
        instances, timings = [], []
        for index, visit in enumerate(visits):
            ...
            instance = ScheduledActivityInstance(
                ...
                defaultConditionId=ids[visits[index + 1].uid] if not has_untimed_visits and index + 1 < len(visits) else None,
                timelineExitId=exit_.id if not has_untimed_visits and index + 1 == len(visits) else None,
                extensionAttributes=[self.extension("scheduledVisit", visit.uid, visit)],
            )
            instances.append(instance)
            if getattr(getattr(visit, "timing_mode", None), "value", getattr(visit, "timing_mode", None)) == "UNTIMED":
                self.issue(
                    "USDM_UNTIMED_VISIT_REQUIRES_MANUAL_DATE", f"study-visits/{visit.uid}/untimed_timing",
                    "ScheduleTimeline",
                    "The source defines manual dates and nominal timing; presentation order does not establish an executable sequence.",
                    "Retain the exact source timing in scheduledVisit metadata and record actual dates manually.",
                )
                continue
